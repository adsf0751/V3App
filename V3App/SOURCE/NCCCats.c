...
ISO_FIELD_TYPE_NCCC_ATS_TABLE srNCCC_SALE_ISOFieldType[] =
{
...
/*
Function        :mySalePackData
Date&Time       :
Describe        :參考myPackData，把原先金額和日期帶入pobTran的值，
                 未來如果可以取得信用卡資訊，可以補齊其他欄位
*/
...
//    inFunc_Sync_BRec_Date_Time(pobTran, &srRTC);
    ...
    for(i=0;i<FieldSz;i++)
    {            
        int Length = srNCCC_SALE_ISOFieldType[i].inFieldLen / 2;
        unsigned char* uszBCD = malloc(sizeof(unsigned char)*(Length+1));
        memset(uszBCD, 0x00, Length+1);
        printf("field num is %d\n",srNCCC_SALE_ISOFieldType[i].inFieldNum);
        /*===== set a bit in the bitmap field =====*/
        /*參考 inNCCC_ATS_BitMapCheck */
        int offset = (srNCCC_SALE_ISOFieldType[i].inFieldNum -1) / 8;
        uszPackBuf[9 + offset] |= (1U << 8-(srNCCC_SALE_ISOFieldType[i].inFieldNum - 8*offset));
        
        switch(srNCCC_SALE_ISOFieldType[i].inFieldNum)
        {
            case 3:
            {
            ...
            case 4:
            {
                /*金額*/
                char amountStr[12+1];
                memset(amountStr,0x00,12+1);
                sprintf(amountStr,"%012ld",pobTran->srBRec.lnTxnAmount);
                printf("amount is %s\n",amountStr);
                inFunc_ASCII_to_BCD(&uszBCD[0],amountStr,Length );
...
{
    printf("Field_4: %s\n",myECRTable[10-1].Data);
    printf("Field_12: %s\n",myECRTable[12-1].Data);
    printf("Field_13: %s\n",myECRTable[11-1].Data);
    printf("Field_35: %s\n",myECRTable[8-1].Data);
    printf("Field_38: %s\n",myECRTable[13-1].Data);
    printf("Field_39: %s\n",myECRTable[15-1].Data);
    printf("Field_41: %s\n",myECRTable[17-1].Data);
    printf("Field_42: %s\n",myECRTable[16-1].Data);
    printf("Field_48: %s\n",myECRTable[18-1].Data);
    int FieldSz = sizeof(srNCCC_ATS_ISOFieldType)/sizeof(srNCCC_ATS_ISOFieldType[0]);
    int i;
    int inCnt = 0;
    //Message Length
...
//              printf("Field_4: %s\n",myECRTable[10-1].Data);
//              inFunc_ASCII_to_BCD(&uszBCD[0],"000000000100",Length );
                inFunc_ASCII_to_BCD(&uszBCD[0],&myECRTable[10-1].Data[0],Length );
                memcpy((char *) &uszPackBuf[inCnt], (char *) &uszBCD[0], Length);
                inCnt += Length;
                break;
            }
...
//                printf("Field_12: %s\n",myECRTable[12-1].Data);
//                inFunc_ASCII_to_BCD(&uszBCD[0],"073012",Length );
                inFunc_ASCII_to_BCD(&uszBCD[0],&myECRTable[12-1].Data[0],Length );
                memcpy((char*)&uszPackBuf[inCnt], (char *)&uszBCD[0],Length);
                inCnt +=3;
                break;
            }
            ...
            {
//                printf("Field_13: %s\n",myECRTable[11-1].Data);
                inFunc_ASCII_to_BCD(&uszBCD[0],&myECRTable[11-1].Data[0],3 );
                memcpy((char*)&uszPackBuf[inCnt], (char *)&uszBCD[0],3);
                inCnt += 3;
                break;
            }
            ...
            {
//                printf("Field_35: %s\n",myECRTable[8-1].Data);
//                uszPackBuf[inCnt++] = (myECRTable[8-1].Length/10)<<4 |(myECRTable[8-1].Length % 10) ;
//                memcpy((char *) &uszPackBuf[inCnt],&myECRTable[8-1].Data[0],myECRTable[8-1].Length);
//                inCnt += myECRTable[8-1].Length;
                uszPackBuf[inCnt++] = 0x021;
                inFunc_ASCII_to_BCD(&uszPackBuf[inCnt], "4761739001010119D15F", 11);
                inCnt += 11;
                break;
...
//                printf("Field_38: %s\n",myECRTable[13-1].Data);
            ...
            {
                //                printf("Field_39: %s\n",myECRTable[15-1].Data);
//                inFunc_ASCII_to_BCD(&uszPackBuf[inCnt], &myECRTable[15-1].Data[0], 2);
                inFunc_ASCII_to_BCD(&uszPackBuf[inCnt], "0000", 2);
                inCnt += 2;
                break;
            }
            case 41:
...
//                printf("Field_41: %s\n",myECRTable[17-1].Data);
                memcpy((char *) &uszPackBuf[inCnt], "13992020", 8);
                inCnt += 8;
//                memcpy((char *) &uszPackBuf[inCnt], &myECRTable[17-1].Data[0], myECRTable[17-1].Length);
//                inCnt += myECRTable[17-1].Length;
                break;
            }
            case 42:
            {
...
//                memcpy((char *) &uszPackBuf[inCnt], szTemplate, strlen(szTemplate));
//                inCnt += strlen(szTemplate);
                memcpy((char *) &uszPackBuf[inCnt], &myECRTable[16-1].Data[0], myECRTable[16-1].Length);
                inCnt += myECRTable[16-1].Length;
                break;
            }
            case 48:
            {
...
//                printf("Field_48: %s\n",myECRTable[18-1].Data);
                ...
                memset(&uszPackBuf[inCnt],' ',6);//Other = No Data ，Set  0x20 *6
                inCnt += 6;
//                memcpy(&uszPackBuf[inCnt],&myECRTable[18-1].Data[0],myECRTable[18-1].Length);
//                inCnt += myECRTable[18-1].Length;
//                char	szTemplate[40 + 1];
//                uszPackBuf[inCnt ++] = 0x00;
//		uszPackBuf[inCnt ++] = 0x19;
//		uszPackBuf[inCnt ++] = 0x12;
