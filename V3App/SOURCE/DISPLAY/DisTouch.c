...
int inDisTouch_Open_TouchFile(int *inTouch_Handle)
{
	...
	if (*inTouch_Handle < 0)
	{
		*inTouch_Handle = open (_TOUCH_FILE_ , O_RDONLY | O_NONBLOCK);
		...
		{
			/* Open fail */
                        printf("TouchFile Open Fail handle:%d\n", *inTouch_Handle);
			/* 開失敗不支援觸控 */
...
int inDisTouch_Read_TouchFile(int *inTouch_Handle)
{
	...
	{       /*因為read參數三原因，所以一次只讀一個事件?*/
		...
		if (inRead_Temp_Length != -1)
		{
			...
			{
                                printf("Read Length 小於一個event %d\n", inRead_Temp_Length);	
...
//		return (_DisTouch_No_Event_);
	...
	for (inEventCount = 0; inEventCount < ginEventNum; inEventCount++)
	{
		...
		else if (EV_SYN == gsrEvent[inEventCount].ushType)
		{
			...
			{
                            printf("EV_SYN:[%d] X:%d y:%d\n", inEventCount, gsrDisTouchObj.inX_Temp, gsrDisTouchObj.inY_Temp);	
                            return (inTouchEvent);
			}
		}
...
int inDisTouch_InArea(DISTOUCH_OBJECT *srDisTouchObj, int inTouchSensorFunc)
{
          	...
          	if (inTouchSensorFunc == _Touch_NEWUI_CHOOSE_HOST_)
 	{
 		...
 		if ((srDisTouchObj->inX_Temp != 0) && (srDisTouchObj->inY_Temp != 0))
 			   ...
 			   (srDisTouchObj->inY_Temp > _DisTouch_NEWUI_FUNC_2_TO_7_3X4_LINE1_Y1_-10) && 
 			   (srDisTouchObj->inY_Temp < _DisTouch_NEWUI_FUNC_2_TO_7_3X4_LINE1_Y2_-10))
                         {
 		...
 		if ((srDisTouchObj->inX_Temp != 0) && (srDisTouchObj->inY_Temp != 0))
 			   ...
 			   (srDisTouchObj->inY_Temp > _DisTouch_NEWUI_FUNC_2_TO_7_3X4_LINE2_Y1_ +20) && 
 			   (srDisTouchObj->inY_Temp < _DisTouch_NEWUI_FUNC_2_TO_7_3X4_LINE3_Y1_))
                         {
